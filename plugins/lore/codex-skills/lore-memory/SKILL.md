...
You interact with Lore through the shared `lore.sh` helper script bundled with this plugin. First locate the installed Lore plugin root in Codex:
...
The helper script reads credentials from the project's `.lore.env` + `.lore.env.local` (walked up from the current working directory — config is **per-project**, so every repo has its own). If the script errors with "Lore not configured for this project" or "no API key set", tell the user to run the `lore-setup` skill in this project and stop.
...
The namespace `_index` is a navigable catalog of every file, with keys, titles, and short descriptions. **In many sessions it has already been pre-loaded for you as `additionalContext` by the `SessionStart` hook** — look for a block titled "Lore wiki index for `<app>/<namespace>`" at the top of the session context. If you see it, use it directly; do **not** call `lore.sh get` redundantly.

If the preamble is absent (or the hook is not installed yet), fetch the index yourself:

```bash
bash "$LORE_PLUGIN_ROOT/scripts/lore.sh" get
```
...
```bash
bash "$LORE_PLUGIN_ROOT/scripts/lore.sh" get <file-key>
```
...
echo 'The content to remember, written as a paragraph or two of markdown.' \
  | bash "$LORE_PLUGIN_ROOT/scripts/lore.sh" remember
```
...
If the script prints an error like `Error: Lore not configured for this project. Run the lore-setup skill to connect it.`, tell the user to run the `lore-setup` skill and stop. Do not attempt to work around it.
