...
There is no global config. Each teammate runs `/lore-setup` in the repo to add their own API key; the app/namespace wiring is inherited from what's already committed.
...
- **Mid-session memory** is available via the `lore-memory` skill — Claude can now read from and write to this project's Lore namespace.
- **SessionStart preload** — the next time you start a Claude Code session in this project, the wiki's `_index` will be pre-loaded into context automatically.
- **Auto-ingest on session end** — every time a Claude Code session ends in this project, the transcript gets posted to Lore and integrated into the wiki.
...
- **Never write at `$HOME`.** `setup.sh` refuses this explicitly. If the user runs `/lore-setup` from their home directory (no project context), tell them to `cd` into a specific project first.
